...
% Sweep Kp and Ki
kp_vec = linspace(0.0001, 0.01, 10);
ki_vec = linspace(1e-6, 5e-4, 10);
stability_map = zeros(10, 10);
...
for i = 1:10
    ...
    for j = 1:10
        ...
        for k = 1:n_aug
            xh = [x_tr(st_cl); 0]; xh(k) = xh(k) + xs;
            xmh = [x_tr(st_cl); 0]; xmh(k) = xmh(k) - xs;
            A(:,k) = (cl_sys(xh) - cl_sys(xmh)) / (2*xs);
        end
        eg = eig(A);
        stability_map(j,i) = all(real(eg) < 1e-6);
    end
end

fprintf('Stability Map (X: Kp, Y: Ki)\n');
disp(stability_map);

[KP, KI] = meshgrid(kp_vec, ki_vec);
save('tmp/sweep_results.mat', 'KP', 'KI', 'stability_map');
